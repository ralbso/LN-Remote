...
class GUI(QMainWindow, LuigsAndNeumannSM10):
    ...
    def moveAway(self):
        if int(self.read_x.text()) < 100:
            self.errorDialog('Pipette might still be in the brain.')

        # first move stage X (1) all the way out
        self.approachAxesPosition(axes=[1], approach_mode=0, positions=[3000], speed_mode=1)
